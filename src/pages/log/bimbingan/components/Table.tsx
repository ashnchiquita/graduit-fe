...
export const CardTable = forwardRef<HTMLDivElement,CardTableProps>(({TableHeader,TableContent,className,...props},ref)=>{
    return(
        <>
            <div className={cn("w-full",className)} {...props} ref={ref}>
                {TableHeader}
                {
                    TableContent.map((data,idx) => {
                        return (
                            <TableRow tabIndex={idx} tanggal={data.tanggal} laporanKemajuan={data.laporanKemajuan} todo={data.todo} status={data.status} rencana={data.rencana} berkas={data.berkas}
                            /> 
                        )
                    })
                }
...
export const TableHeader = forwardRef<HTMLDivElement,TableHeaderProps>(({sort,className,...props},ref)=>{
    return (
    <>
        <div className={cn("grid grid-cols-8 gap-1 bg-slate-50 py-3 px-4 border-y-2 border-y-[#EAECF0] text-xs font-medium text-[#667085] justify-center",className)} ref={ref} {...props}>
            <div className="col-span-1 flex gap-2 items-center"><p>Tanggal</p><ButtonIcon sort={sort}/></div>
...
export const Badge = forwardRef<HTMLDivElement,badgeProps>(({status = 'Sah',size,variant,className,...props},ref)=>{
    return (
        <>
            <div className={cn(badgeVariants({variant,size,className}))} {...props} ref={ref}>
                <span className={`flex w-2 h-2 me-2 ${status == 'Sah' ? 'bg-[#037847]' : (status == 'Menunggu' ? 'bg-amber-400' : 'bg-red-500')} rounded-full`}></span>
...
const ButtonDownload = forwardRef<HTMLDivElement,ButtonDownloadProps>(({berkas,className,...props},ref)=>{
    return (
        <>  
            <div className={cn("flex flex-col xl:flex-row items-center gap-2 justify-center bg-[#F1F5F9] w-fit py-2 px-3 rounded-sm",className)} {...props}>
...
export const TableRow = forwardRef<HTMLDivElement,TableRowProps>(({tanggal,laporanKemajuan,todo,berkas,status,rencana,className,...props},ref)=>{
    return(
        <>
            <div className={cn("grid grid-cols-8 gap-1 py-3 px-4 border-b-2 border-b-[#EAECF0] text-xs text-[#667085] justify-center",className)} {...props} ref={ref}>
                ...
                <div className="col-span-1"><Badge status={status} variant={status == 'Sah' ? 'default' : (status == 'Menunggu' ? 'warning' : 'danger')}/></div>
                <div className="col-span-1">{rencana}</div>
            </div>
        </>
    )
