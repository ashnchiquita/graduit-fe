...
export default function TopicAllocation() {
  ...
  if (isLoading) {
    ...
    return (
      <>
        <main className="flex min-h-full min-w-full flex-col p-5 md:min-w-0">
          <DataTable
            table={table}
              ...
              isAbleToCreate ? (
                <>
                  <Button
                    size="sm"
                    onClick={() =>
                      isAbleToCreate ? handleClickImportFromTemplate : undefined
                    }
                    className="hidden gap-2 border border-blue-500 bg-white px-2 py-1 hover:border-blue-600 hover:bg-gray-300 md:flex"
                  >
                    <Download color={"blue"} size={16} />
                    <div className="hidden md:block md:text-blue-500">
                      Import Topik (.xlsx)
                    </div>
                  </Button>
